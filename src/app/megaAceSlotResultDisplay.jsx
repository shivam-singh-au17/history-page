...
const MegaAceSlotResultDisplay = ({ roundData }) => {
  ...
  const parseGameData = () => {
    ...
    return {
      ...
      baseGame: baseData
        ? {
            reelViews: baseData.cascadeData.map((cascade, index) => ({
              view: cascade.reelView,
              winAmount: cascade.spinWin || 0,
              winningSymbols: cascade.blustSymbols || [],
              multiplier:
                index === 0 ? 1 : index === 1 ? 2 : index === 2 ? 3 : 5,
              scatterPositions: cascade.scatterPositions || [],
            })),
            totalWin: baseData.totalWin || 0,
            multiplier: baseData.winMultiplier,
          }
        : null,
      freeSpins: freeSpinsData.map((spin, index) => ({
        spinNumber: index + 1,
        reelViews: spin.cascadeData.map((cascade, idx) => ({
          view: cascade.reelView,
          winAmount: cascade.spinWin || 0,
          winningSymbols: cascade.blustSymbols || [],
          multiplier: idx === 0 ? 2 : idx === 1 ? 4 : idx === 2 ? 6 : 10,
          scatterPositions: cascade.scatterPositions || [],
        })),
                  ...
                  <span className="multiplier-chip">
                    {spin.reelViews
                      ...
                      .map(
                        (reelData, index) =>
                          `${reelData.multiplier}${
                            index < spin.reelViews.length - 2 ? "x, " : "x"
                          }`
                      )}
                  </span>
                )}
        ...
        <div className="summary-grid">
          <div className="summary-item">
            ...
            <div className="summary-value">
              {currency} {(reelData.winAmount / reelData.multiplier).toFixed(2)}
            </div>
          </div>
          <div className="summary-item">
...
export default MegaAceSlotResultDisplay;
